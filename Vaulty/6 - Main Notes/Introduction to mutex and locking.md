...
The standard library makes sure that when a thread tries to lock a mutex that's already locked it can't, only when it's **unlocked** is that possible. This make sure that threads have self-consistent view on shared data.
