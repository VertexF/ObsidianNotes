...
$$L_o(P, \vec{\omega_o}) = L_e(P, \vec{\omega_o}) + \int\limits_{S^2} f_r(P,\vec{\omega_o}, \vec{\omega_i}) L_i(P,\vec{\omega_i}) | \cos(\theta_i) | d\vec{\omega_i}$$
...
Whitted's ray-tracing algorithm simplifies this integral by incoming light from most direction only in $L_i(P,\vec{\omega_i})$ (**light incident radiance**) for direction to light sources and directions of perfect reflections and refractions. In short this turns the integral into a small sum over a small number of directions.
