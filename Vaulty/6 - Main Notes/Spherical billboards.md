2026-07-09 09:22
Status: #baby 
Tags: [[graphics theory]] [[particles]]
# Spherical billboards

Spherical billboard are defined by a centre point $p$ and two radii $r_{x}$ and $r_{y}$ equal to the half width and height. The billboard can rotate about, but the orientation of the normal vector needs to always face the camera. So the union of all possible orientations is a sphere of radius $\sqrt{r^2_{x} + r^2_{y}}$ 

We need to move the billboard vertices into billboard object space. Lets set this up so $M_{c}$ which will be the transform to camera space to world space, and $M_{o}$ which will be the transform from billboard object space to world space.

To connect both spaced $M_{c}$ and $M_{o}$ together, you want to get both vectors $\vec{x}$ and $\vec{y}$ to be the postive $x$ and $y$ axes of the camera matrix given by
...
If we have four vertex position $v_{0}$, $v_{1}$, $v_{2}$ and $v_{3}$ and apply the transform based on the x-y of the camera you get these four equation.
...
Note, we are doing a plane equation over 2 vectors to transform these points. If $M_{o}$ contains a scale then $\vec{x}$ and $\vec{y}$ these need to be normalised. For objects made of a single billboard. 

For a single billboard it is often the case that $p$ lies at the object-space origin. This means that the vertex positions are simply a sums of differentces of $r_{x}\vec{x}$ and $r_{y}\vec{y}$. (This is what we do in our code below) This doesn't work for particle systems that contains a large number of independent billboards, when you have billboards that have point $p$ that isn't represented at the centre each billboard.
$$v_{0} = p - r_{x}\vec{x} + r_{y}\vec{y} \space \space \space \space \space v_{1} = p + r_{x}\vec{x} + r_{y}\vec{y}$$
$$v_{2} = p + r_{x}\vec{x} - r_{y}\vec{y} \space \space \space \space \space v_{3} = p - r_{x}\vec{x} - r_{y}\vec{y}$$
...
[[What are billboards]]
#### Source Notes
[[Foundations of Game Engine Development - Graphics]]
