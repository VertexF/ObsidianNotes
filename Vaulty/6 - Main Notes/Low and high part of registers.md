...
Registers store 2 bytes in them. If we are encoding registers that take 16-bits we then copy the all the data, or when we are copying bytes we use either the higher or lower parts of the a register, if we copying the 8-bits we need to encode either the low or the high parts of the register. This is done with
