...
To see why the samplying distributions reduces variances let first look at a function that has a proporation relationship with a PDF $p(x) \propto f(x)$ or if $f(x)$ times a constant is equal to the PDF $p(x) = cf(x)$ To normalize the PDF it requires us to
$$c = \frac{1}{\int f(x)dx}$$
...
The variance here is zero which is wrong. Since if we could just integrate the integal directly. However, if the density $p(x)$ can be found that is similar to the shape to $f(x)$ the variance is reduced. 
