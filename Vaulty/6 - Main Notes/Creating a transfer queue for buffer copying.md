...
When you want to transfer data from one buffer to another you'll need to do that with a command on a command buffer. Meaning you'll need a queue that supports the transfer commands. You can just use the graphics queue as these support transfer commands. I didn't made this work on it's own transfer queue.

To be able to do this you need to follow these steps.
1) You need to look for a queue that's either a VK_QUEUE_GRAPHICS_BIT or just a VK_QUEUE_TRANSFER_BIT. This is done as part of [[Checking for family queue suitability]]. So you should at least two different queue families indices. 
