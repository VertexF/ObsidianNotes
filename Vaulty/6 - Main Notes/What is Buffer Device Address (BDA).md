...
You access the data in the shaders with a 64-bit integer (8 bytes) so basically a pointer. You're job after creating the buffer is getting the buffer device address pointer to the shader were the buffer is used. This can be done with a push constants as you have 128 bytes of memory to work with and 256 bytes on vulkan 1.4

The buffer device address itself isn't a new type of buffer, it's just the way you access the buffer. You can access a buffer in more than 1 way. So calling it a "buffer device address buffer" make no sense.
# References
##### Main Notes
