...
You can send up an image to the shaders and read it directly, however that's never really done with textures. Typically you'll want a sampler that applies filters and transforms that computes the final image.
...
Samplers also details what happens when you sample outside the texture on geometry, in the samplers **address mode**. Here are pictures of the possible abilities
