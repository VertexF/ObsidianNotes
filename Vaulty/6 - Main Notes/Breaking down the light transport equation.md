...
So by selectin a random direction of $\vec{\omega'}$ we have equal propability over all possible directions. After that we can estimate the integral and we can compute the weighted product of the BSDF which describe the light scattering properties of the material a point **P**, the $L_i(P,\hat{\omega_i})$ the **incident spectral radiance** and the cosine factor for the angle of the surface at point **P**
...
Solving the **BSDF** and the **cos($\theta$)** leaving us with the $L_i(P,\vec{\omega_i})$ the **incident spectral radiance** function to work out. To work this out we actually have to run the same process again at the origin at were the **incident special radiance** is, recursive repeat the process until we hit a certain depth of recursion.

To work out the **bidirectional scattering distribtion function (BSDF)** this function is working out how much that is scattered returns to our camera we need to get the texture and the surface properties for this to work.
