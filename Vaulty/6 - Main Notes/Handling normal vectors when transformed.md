...
This is because you consider the partial derivatives that compose a normal vector, which consider a point on a surface $(\delta f / \delta x, \delta f / \delta y, \delta f / \delta z)$ the distance $x, y, z$ are all in the denominator of the vector. Since this is so different we write the normal vector a row matrix rather than a column matrix for a regular vector.
