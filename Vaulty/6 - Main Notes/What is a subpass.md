...
A subpass is part of the render pass that sets up the dependencies with attachments between the subpasses and itself. Command buffers are recorded into a subpass when using a render pass instance when using graphics commands. Meaning that actual operations from the command buffer get executed here. 
...
Remember we are **NOT** actually writing the output of the operations to a subpass, they are interested in ordering graphics command and other operations.
