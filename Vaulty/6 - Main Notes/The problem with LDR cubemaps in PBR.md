...
So it is very important for PBR that you deal with HDR properly both for colour [[Linear lighting vs High Dynmanic Range rendering]] and your vulkan swapchain is in the colour space format you expect [[Swapchain colour space format]]. This is because the PBR taking is taking in real physically properties. Without working in HDR you can't work out the relative intensity of something like a small lightblub and the sun for example.

So with image based lighting that's based on the environment's indirect light intensity on the colour values of a cubemap. We need some way to store the lighting's high dynamic range into a the cube.
