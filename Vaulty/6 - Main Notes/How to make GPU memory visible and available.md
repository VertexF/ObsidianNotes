...
Below is an example of how to take things from the L1 cache and make it **available** to the L2 cache for a colour attachment.
```c++
...
Using the destination flags we are moving things from the L2 to L1 making the memory **visible**
```c++
...
For example, lets say we have `vkCmd1` that writes out colour memory to it's L1 cache. `vkCmd2` wants to do something with memory when`vkCmd1` has to finish. So we need to pull the memory from L2 cache to the L1 cache.
