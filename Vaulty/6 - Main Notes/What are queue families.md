...
Each GPU you select will have a different number of queue families you can use. It's important to select a queue family that can create a queue that allows you to do the commands you want. For example, if you have a queue that only support memory transfers, you wouldn't be able to submit graphics and compute commands to it.

Remember a single queue family can have more than one queue type. It's common for GPUs to have a queue family with everything including the graphics queue and a separate transfer queue for example.
# References
##### Main Notes
