...
It might helpful to think about this as a `move ax = bx` here we are using the = so to show that **bx** is the source.

How this will run on the CPU is like that
1) It runs the code through an assembler
2) That encoded it to binary that the x86 chip **instruction decoder** understands
