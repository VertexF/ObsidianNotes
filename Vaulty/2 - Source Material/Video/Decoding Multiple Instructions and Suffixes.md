...
##### The byte encoded
When you have the move bytes like this `10010`**DW** the**W** is for wide 8/16-bit if the **D** is set to `1` then we are doing a **load** because the register goes first, if the **D** is `0` then we are doing a **store**.

In the second byte MOD REG R/M to encode a **store** were the register is second, you need both the **MOD** and the **R/M** field to encode what the memory is.
...
You can also do this for **effective address look** up 
```c
mov ax, [bx + di - 37]
```
